---
title: "使用 shadcn/ui 前該補的 TailwindCSS 基礎知識(三) - TailwindCSS v4 內建 Utility Classes 速查"
slug: tailwindcss-knowledge-before-shadcn-ui-3
authors: bosh
description: 這篇技術筆記整理了 TailwindCSS v4 的核心語法速查，涵蓋顏色系統、字型設定、間距配置、版面布局、響應式設計等基礎概念，並提供實用的程式碼範例，幫助開發者快速掌握 TailwindCSS 的使用方式，為學習 shadcn/ui 打下紮實基礎。
keywords: [shadcn/ui, TailwindCSS]
tags: [shadcn/ui, TailwindCSS]
date: 2025-11-06
image: https://res.cloudinary.com/djtoo8orh/image/upload/v1768927407/Docusaurus%20Blog/Blog/%E4%BD%BF%E7%94%A8%20shadcn%20%E5%89%8D%E8%A9%B2%E8%A3%9C%E7%9A%84%20TailwindCSS%20%E5%9F%BA%E7%A4%8E%E7%9F%A5%E8%AD%98/tailwindcss_v4_cheat_sheet_cover_iyxtdv.png
---

![](https://res.cloudinary.com/djtoo8orh/image/upload/v1768927407/Docusaurus%20Blog/Blog/%E4%BD%BF%E7%94%A8%20shadcn%20%E5%89%8D%E8%A9%B2%E8%A3%9C%E7%9A%84%20TailwindCSS%20%E5%9F%BA%E7%A4%8E%E7%9F%A5%E8%AD%98/tailwindcss_v4_cheat_sheet_cover_iyxtdv.png)
...
> 3. **TailwindCSS v4 內建 Utility Classes 速查（本篇）**
...
這篇文章介紹的所有 utility classes 都是 TailwindCSS **預設內建**的，只要安裝 TailwindCSS 就能直接使用，不需要任何額外配置。

如果你想了解如何透過 `@theme` 自訂設計系統（例如 shadcn/ui 的語意化顏色），請參考下一篇：[透過 @theme 自訂設計系統](./tailwindcss-knowledge-before-shadcn-ui-4)。
...
- **灰階系列**：`slate`、`gray`、`zinc`、`neutral`、`stone`
- **彩色系列**：`red`、`orange`、`amber`、`yellow`、`lime`、`green`、`emerald`、`teal`、`cyan`、`sky`、`blue`、`indigo`、`violet`、`purple`、`fuchsia`、`pink`、`rose`

> 完整顏色參考：[TailwindCSS Colors 官方文件](https://tailwindcss.com/docs/customizing-colors)
...
:::tip[**兩種透明度的差異**]
- `bg-blue-500/50`：只有背景色是半透明的，文字和其他內容不受影響
- `opacity-50`：整個元素（包括文字、圖片、子元素）都會變成半透明
:::
...
:::warning[**重要觀念**]
沒有前綴的 class 會套用到**所有尺寸**，帶前綴的 class 會從該斷點**往上**覆蓋。
:::
