...
前陣子在研究一些 git 專案開發流程的最佳實踐，比如如何遵循 **Conventional Commits** 規範撰寫 commit message，還有依照 **Semantic Versioning** 原則來管理版本號等等。這段時間我看了滿多 Github 上知名度高的開源專案，並試著從這些專業的開源專案的 commit 歷史、PR 紀錄 、Change log、Release notes 中學習如何建立一套規範化的工作流。
...
為了方便在專案中實踐上述的工作流，我研究了許多工具。在研究過程中我偶然發現，不少開源專案的 commit message 裡都有搭配 emoji，看起來挺直觀又滿有趣的，於是稍微查了一下，才知道這東西叫做 **Gitmoji** ，於是便順道一起研究如何把 Gitmoji 也加入到上述的工作流中。

礙於篇幅的原因，我將以上三個工作階段分成三篇文章來介紹，每篇都詳細講解在有使用 Gitmoji 與沒有使用 Gitmoji 的情況下，該如何安裝與配置所需要的工具，以及實際使用的效果。
...
你有沒有遇過這樣的情況：打開 Git 提交記錄，看到的都是「Fix bug」「Update」「Change」這類模糊不清的訊息？完全無法理解這些提交到底改了什麼，或者影響了哪些地方。其實，對於一個好的多人協作專案來說，提交訊息不只是用來記錄改動的工具，還是團隊協作和專案管理的重要一環。如果提交訊息清晰規範，大家都能快速看懂改了什麼，為什麼改，甚至能用它來自動生成變更日誌，節省不少手動整理的麻煩。

這就是 [**Conventional Commits**](https://www.conventionalcommits.org/en/v1.0.0/) 出現的原因。它提供了一套簡單又實用的規範，比如用固定格式的「標籤」來描述提交的類型（像 `feat` 表示新增功能，`fix` 表示修復問題），這樣不但提升提交訊息的可讀性，甚至能結合工具來做到自動生成變更日誌，或自動升級專案版本好等一些方便的功能。

> 如果對 **Conventional Commits** 有興趣想要進一步深入了解，歡迎參閱我之前寫的這篇文章：[**Conventional Commits 的實踐指南：寫出乾淨的提交訊息**](http://localhost:3000/docs/DevTools/Git/conventional-commits)
> 

### **Commitizen：規範化提交的好幫手**

提交訊息規範化聽起來挺有道理，但實際上要執行起來，很多人還是會感到麻煩：每次提交都要手動寫那麼多規範的內容？也太費時了吧！
...
不過，在介紹 **Commitizen** 搭配 **Gitmoji** 的實作之前，讓我們先來了解什麼是 **Gitmoji**，以及它為什麼可以讓提交訊息變得更有趣又更具表達力。
...
如果你在導入這套流程時有其他的需求或改進想法，別忘了靈活調整工具和設定，讓它更貼合你的專案需求。希望這篇文章能夠幫助到你，並為你的開發流程增添一點小小的效率和樂趣！
