...
這表示我們可以找到 **2 個線性獨立**的特徵向量對應於 $\lambda = 5$。
...
這表示雖然 $\lambda = 2$ 是二重根，但我們只能找到 **1 個**線性獨立的特徵向量，特徵空間只是一條線而非一個平面。
...
這些不變量揭示了線性變換的「內在性質」，不會因為我們選擇的座標系統而改變。這也是為什麼特徵值如此重要——它們是線性變換的**本質特徵**。
:::
