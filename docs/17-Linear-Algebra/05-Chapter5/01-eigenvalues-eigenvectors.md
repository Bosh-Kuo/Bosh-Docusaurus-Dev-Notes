...
這意味著 $\lambda_1 \mathbf{x} = \lambda_2 \mathbf{x}$，即 $(\lambda_1 - \lambda_2)\mathbf{x} = \mathbf{0}$。
...
這意味著**同方向上的所有非零向量**都是相同特徵值的特徵向量。這就是為什麼我們在描述特徵向量時，通常只給出一個「代表」，其他都是它的純量倍數。
...
這也解釋了為什麼特徵向量「不唯一」——因為任何純量倍數（甚至線性組合）都還是特徵向量！
:::
