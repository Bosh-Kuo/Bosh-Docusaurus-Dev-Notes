...
:::tip 多重開機（Multi-Boot）：GRUB 如何讓一台電腦選擇不同 OS 開機
GRUB「在多個核心映像之間選擇」的能力，正是**多重開機（Multi-Boot / Dual-Boot）** 系統的實作基礎。要讓一台電腦能在開機時自由選擇不同的 OS，需要兩個條件配合：

**1. 磁碟分割**：每個 OS 安裝在各自獨立的磁碟分割區（Partition）中，擁有自己的根檔案系統與核心映像檔，彼此互不干擾。

**2. 具備多重開機能力的 Bootstrap Program**：Bootstrap Program 必須能夠識別多個 OS，並在開機時呈現選單讓使用者選擇。為了做到這件事，它需要：
- **讀取組態檔**，其中記錄了所有已安裝 OS 的核心映像路徑（`BOOT_IMAGE`）與對應根分割的 UUID（`root`）
- **呈現開機選單**，等待使用者選擇（或計時後自動載入預設 OS）
- **依選擇載入對應核心**，並掛載該 OS 的根分割，確保核心存取的是自己的檔案系統，而非其他 OS 的分割區

以 GRUB 為例，整個多重開機機制完全由一份組態檔驅動，管理員只需修改該檔案即可新增或移除可開機的 OS，不需要重新燒錄韌體。
:::
