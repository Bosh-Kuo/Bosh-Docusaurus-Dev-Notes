...
:::note 「浪費資源」的設計哲學
OS 的設計原則之一是有效率地使用硬體資源，但這條原則並非放諸四海而皆準。**當系統只為單一使用者服務時，主動「浪費」資源反而是合理的設計決策。**

以個人電腦為例：CPU 閒置等待使用者輸入、或為了讓 GUI 動畫更流暢而預留大量記憶體，表面上看來是資源浪費，但實際上並非如此——**因為沒有其他使用者在競爭這些資源**，閒置資源讓它空著本來就沒有損失。此時提升使用者體驗才是系統的真正目標，資源效率退居次位。這與伺服器系統截然相反：伺服器同時服務數百個用戶，任何一點資源浪費都會壓縮其他人的份額。
:::
