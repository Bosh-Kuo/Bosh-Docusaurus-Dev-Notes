...
> S.O.L.I.D 軟體架構中的 D 代表就是**依賴反轉原則 (Dependency Inversion Principle, DIP)** ，在物件導向設計 (OOD) 中是個非常重要的設計原則。這個原則與我們經常提到的**控制反轉 (Inversion of Control, IoC)** 和 **依賴注入 (Dependency Injection, DI)** 緊密相關，這些概念共同構成了一個強大且靈活的架構基礎。本篇文章的目標是讓對 OOP 與 Design Pattern 不熟悉的人也能對以上三個觀念建立清楚的理解。
>


## **快速認識依賴注入 (DI)、控制反轉 (IoC)、依賴反轉原則 (DIP)**

大部分介紹 DI, IoC, DIP 的文章開頭就開始大談「依賴」、「耦合」、「抽象」、「高低階模組」、「容器」、「注入」等 OOP 中的專有名詞。對於剛開始接觸 OOP 的人來說，光看這些詞都看不太明白了，更遑論要用這些不懂的名詞來建立對 DI, IoC, DIP 的理解了。

因此我決定在這篇文章的開頭以最白話的方式先帶大家建立對 DI, IoC, DIP 的概念，讓大家能先理解它們背後的核心想法，而不被繁複的專業術語所困擾。在後續的章節中，我們再深入探討這些名詞和技術細節，以便完整地掌握這些設計原則。
...
來談談什麼是依賴反轉原則吧。它是 OOP 中一個非常重要的設計思維，[SOLID Principles of Object-Oriented Design and Architecture](https://www.udemy.com/course/solid-principles-object-oriented-design-architecture) 這本書提到，DIP 有兩個關鍵原則：
...
舉一個具題的生活情境來比喻，想像你是一個遠程工作者，平常用筆電工作，但突然有一天你只能用手機辦公了，結果你發現每次換工作設備時，都得修改你自己的工作流程。那這樣可不行啊！這就是高階模組依賴低階模組的典型例子，因為你的工作流程直接依賴於具體的設備。
