...
description: 本片文如何在一個前後端專案中設置 ESLint, Prettier, Husky, lint-staged
last_update:
  date: 2023-04-24
keywords:
  - 開發工具
...
為了改善這種情況，我決定幫公司導入 `ESLint`、`Prettier`、`Husky` 和 `lint-staged` 等工具來統一程式碼風格和格式規範，在編輯頁面就讓所有咖發者使用相同的 coding style 與 format ，並且在提交程式碼之前進行檢查，強制 push 到 Github/GitLab 的 Code 都是採用相同的風格與規範，從而保證程式碼品質和提高團隊開發效率。我將導入的過程與查詢的資料記錄下來，方便未來創建新專案時可以 follow 相同的步驟導入這些工具。
...
`ESLint` 是一個靜態程式碼分析工具，用於檢查 JavaScript 程式碼中的錯誤和風格問題。它可以幫助開發者在開發過程中發現和修復程式碼問題，從而提高程式碼質量和開發效率。`ESLint` 可以通過定義開頭為 `.eslintrc` 的配置文件進行自定義設置，例如指定需要檢查的規則、忽略某些檔案或目錄等，同時可以通過 .eslintignore 文件來排除不需要進行 Linter 檢查的文件或文件夾
...
- **`plugins`**: 用於引入自定義的 ESLint 插件，例如 **eslint-plugin-react**、**eslint-plugin-vue** 等等。**plugins** 適合用於導入一個或多個特定領域的 ESLint 檢查規則，例如 React、Vue 等等。這樣可以讓開發者使用 ESLint 檢查特定領域的代碼，從而更加準確地進行代碼檢查。
:::
...
// .prettierrc
{
  ...
  "tabWidth": 2,  // 設置縮進的寬度。
  "trailingComma": "all",  // 控制是否在多行代碼的最後一行添加逗號。
...
`Husky` 是一個 Git hook 工具，它可以在 Git 提交前執行指定的腳本，進行一些額外的檢查或操作，例如在提交前運行 linter 或測試。`lint-staged` 是一個工具，可以在 Git 暫存區 (staging area) 中選取指定的文件，然後對它們進行操作，通常是運行 linter 或者格式化程式碼。 `Husky` 可以與 lint-staged 結合使用，用來確保只有符合規範的程式碼才會被提交到版本庫中，提高團隊協作的程式碼品質。在結合使用時， `Husky` 會在提交前運行 `lint-staged`，然後再進行提交。
...
# 添加一個 Git hook，每次在提交代碼之前運行 Lint-staged
